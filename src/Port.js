...
let portSizeNum;
let portWidthNum;
export default function Port(SampleBase)  {
   
    const[assignedlines,setAssignedlines]=useState(false)
//Initializes the ports for the diagram
async function changenoofinputsfun(args) {
    console.log(args.itemData.text)
    if(args.itemData.text==="input2"){
       await  setAssignedlines(true)
    }
    if(args.itemData.text==="input3"){
       await setAssignedlines(false)
    }    // else setAssignedlines(false)
    // diagramInstance.dataBind();
// if()
await diagramInstance.add(connectors);
...
let input2='M49.5,76.5h-28v-56h28c15.5,0,28,12.5,28,28v0C77.5,64,65,76.5,49.5,76.5z M83.5,48.5h16 M1.5,32.5h20   M1.5,65.5h20 M80.5,45.5c-1.7,0-3,1.3-3,3s1.3,3,3,3s3-1.3,3-3S82.2,45.5,80.5,45.5z'
let input3=`M21.71,76.5h0a83.59,83.59,0,0,0,0-55.9l0-.1h1.58a66.37,66.37,0,0,1,54.1,28h0a66.37,66.37,0,0,1-54.1,28Z M 77,48.5 L 100,48.5 Z M 0,32.5 L 25,32.5Z M 0,65.5 L 25,65.5`
let shape1 = {  type: 'Path',
data: assignedlines?input2:input3
...
let nodes = [
    ...
    },
    {
        ...
        offsetX: 300,
        offsetY: 100,
        annotations: [{ content: "", margin: { left: 5, right: 5 } }],
        ports: node2Port,
        shape:{  type: 'Path',
        data: assignedlines?input2:input3
...
let inputoptions=[
    {value:input2,text:"input2"},
    {value:input3,text:"input3"}
]
    ...
    // }
        ...
        <div className="col-lg-4 property-section">
          ...
          <div className="property-panel-content">
            ...
            {/* <input type="select" values={"two","three"}></input> */}
            


            <DropDownListComponent id="noofports" enabled={true} placeholder="Select no of inputs" dataSource={inputoptions} value="input2" fields={{ value: "value", text: "text" }} change={changenoofinputsfun} />
...
// function changenoofinputsfun(args) {
//     console.log(args.value)
//     let node = diagramInstance.selectedItems.nodes[0]
//     console.log(node.properties.shape.properties)

//     node.properties.shape.properties.shape="path"
//     node.properties.shape.properties.type="path"
//     node.properties.shape.properties.data=args.value
//     diagramInstance.dataBind();
//     console.log(node.properties.shape.properties)
    // let port = getPort();
    // for (let j = 0; j < port.length; j++) {
